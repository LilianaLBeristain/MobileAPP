      ...
      <View style={main.lessonContentArea} >
        <View style={styles.introductionScreen}>
          <Video 
              ref={video}
              useNativeControls
              source={{ uri: 'https://immersio.io/learn/wp-content/uploads/h5p/content/6/videos/sources-6124360d43789.mp4', }}
              resizeMode="cover"
              shouldPlay={playVideo}
              style={styles.image}
              onPlaybackStatusUpdate={(playbackStatus) => onPlaybackStatusUpdate(playbackStatus)}
          />
          { isVisible.one &&<Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:220, fontWeigth:'bold'}}>nitišinihkâson</Text> }

          { isVisible.visible && <Text style={{fontWeight:'bold', fontSize:16, color: '#777', alignSelf:'center', padding:50,paddingBottom: 0}}>nitišinihkâson</Text> }
          { isVisible.visible && <Text style={{fontSize:16, color: '#777', alignSelf:'center'}}>{lessonIntroDescription}</Text> }
          { isVisible.visible && <Text style={{fontSize:16, color: '#777', alignSelf:'center', padding:50, textAlign:'center', paddingTop:20}}>Usually, you say your name before this phrase: [name + nitišinihkâson]</Text> }
        </View>
      </View>

      { isVideo.finished && <View style={main.lessonNavButtonArea}> 
        <Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:-30}}>nitišinihkâson</Text>
        ...
        { isVisible.visible && <TouchableOpacity style={{position: "absolute",
          ...
          disabled={disable}
                onPress={()=>{
                  navigation.navigate("What is your name?")
                  _storeData('done'); 
                }}>
          <Image 
