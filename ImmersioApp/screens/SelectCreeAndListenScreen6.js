      ...
      <View style={main.lessonContentArea} >
        <View style={styles.introductionScreen}>
          <Video 
              ref={video}
              useNativeControls
              source={{ uri: 'https://immersio.io/learn/wp-content/uploads/h5p/content/6/videos/sources-61245491ef955.mp4', }}
              resizeMode="cover"
              shouldPlay={playVideo}
              style={styles.image}
              onPlaybackStatusUpdate={(playbackStatus) => onPlaybackStatusUpdate(playbackStatus)}
          />
          { isVisible.one &&<Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:220, fontWeigth:'bold'}}>Nešta kîla</Text> }

          { isVisible.visible && <Text style={{fontSize:20, marginTop:50, alignSelf:'center', marginBottom: 10}}>nešta kîla = and you</Text> }

        </View>
      </View>

      { isVideo.finished && <View style={main.lessonNavButtonArea}> 
        <Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:-30}}>Nešta kîla</Text>
