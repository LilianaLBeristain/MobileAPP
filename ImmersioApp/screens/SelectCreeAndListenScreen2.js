...
function App() {
  ...
  var lessonIntroDescription = "In the context of this conversation tani means 'what'"
  ...
  async function playSound() {
    ...
    const {sound} = await Audio.Sound.createAsync(require('../assets/Cree/tani_e-isinhkasoyan.mp3'));
    setSound(sound);
    console.log('playing sound');
    await sound.playAsync();

    trackVisible({visible: true, one:false})
  }
    ...
    <SafeAreaView style={main.safeAreaContainer}>
      ...
      <View style={main.lessonContentArea} >
        <View style={styles.introductionScreen}>
          <Video 
              ref={video}
              source={{ uri: 'https://immersio.io/learn/wp-content/uploads/h5p/content/6/videos/sources-6124513ff236f.mp4', }}
              useNativeControls
              resizeMode="cover"
          ...
          />
          { isVisible.one &&<Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:220, fontWeigth:'bold'}}>tâni e-išinihkâsoyan?</Text> }

          { isVisible.visible && <Text style={{padding:50, paddingBottom: 20,fontSize:16, color: '#777', textAlign:'center'}}>{lessonIntroDescription}</Text> }
          { isVisible.visible && <Text style={{fontWeight:'bold', fontSize:16, color: '#777', alignSelf:'center'}}>tâni e-išinihkâsoyan?</Text> }
          { isVisible.visible && <Text style={{fontSize:16, color: '#777', alignSelf:'center'}}>means "what is your name?"</Text> }
          
        </View>
      </View>

      { isVideo.finished && <View style={main.lessonNavButtonArea}> 
        <Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:-30}}>tâni e-išinihkâsoyan?</Text>
        ...
        { isVisible.visible && <TouchableOpacity style={{position: "absolute",
          ...
          disabled={disable}
                onPress={()=>{
                  navigation.navigate("nitišinihkâson")
                  _storeData('done'); 
                }}>
          <Image 
