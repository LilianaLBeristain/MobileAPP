...
function App() {
  ...
  var lessonIntroDescription = "Tâni is a question word that means how."
  ...
  async function playSound() {
    ...
    const {sound} = await Audio.Sound.createAsync(require('../assets/Cree/tani.mp3'));
    setSound(sound);
    ...
    <SafeAreaView style={main.safeAreaContainer}>
      ...
      <View style={main.lessonContentArea} >
        <View style={styles.introductionScreen}>
          <Video 
              ref={video}
              source={{ uri: 'https://immersio.io/learn/wp-content/uploads/h5p/content/6/videos/sources-6124385dc325d.mp4', }}
              useNativeControls
              resizeMode="cover"
          ...
          />
          { isVisible.one &&<Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:220, fontWeigth:'bold'}}>tâni</Text> }
          { isVisible.visible && <Text style={{padding:50, paddingBottom: 0,fontSize:16, color: '#777', alignSelf:'center'}}>{lessonIntroDescription}</Text> }
          { isVisible.visible && <Text style={{fontSize:16, color: '#777', alignSelf:'center', padding:50, textAlign:'center', paddingTop:20}}>However, it can be used in variety of contexts to mean what, depending on the context of the conversation and how the word is used in a sentence.</Text> }
        </View>
      </View>

      { isVideo.finished && <View style={main.lessonNavButtonArea}> 
        <Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:-30}}>tâni</Text>
