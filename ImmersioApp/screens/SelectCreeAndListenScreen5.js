...
function App() {
  ...
  async function playSound() {
    ...
    const {sound} = await Audio.Sound.createAsync(require('../assets/Cree/MooseFactory.mp3'));
    setSound(sound);
  ...
  return (
    <SafeAreaView style={main.safeAreaContainer}>
      ...
      <View style={main.lessonContentArea} >
        <View style={styles.introductionScreen}>
          <Video 
              ref={video}
              useNativeControls
              source={{ uri: 'https://immersio.io/learn/wp-content/uploads/h5p/content/6/videos/sources-6124533cb07c0.mp4', }}
              resizeMode="cover"
              shouldPlay={playVideo}
              style={styles.image}
              onPlaybackStatusUpdate={(playbackStatus) => onPlaybackStatusUpdate(playbackStatus)}
          />
          { isVisible.one &&<Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:220, fontWeigth:'bold'}}>Moose factory nitôhcîn</Text> }

          { isVisible.visible && <Text style={{fontSize:16, color: '#777', textAlign:'center', padding:50,paddingBottom: 0}}>In the context of this conversation, Ben says he is from Moose factory</Text> }
      ...
      { isVideo.finished && <View style={main.lessonNavButtonArea}> 
        <Text style={{fontSize:20,position: "absolute", alignSelf:'center', top:-30}}>Moose factory nitôhcîn</Text>
