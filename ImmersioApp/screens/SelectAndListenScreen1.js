...
function SelectAndListenScreen1() {
  const navigation = useNavigation();
  //TODO: insert code to retrieve description and image from database
  var lessonIntroDescription = "So, to ask what something is you say 'quid est?', meaning 'what is it?'"
  ...
  async function playSound() {
    ...
    const {sound} = await Audio.Sound.createAsync(require('../assets/quid-est-explain.mp3'));
    setSound(sound);
        ...
        <Text style={{color:'gray', fontSize:18, marginTop:25, marginBottom:15}}>Click to listen, and repeat</Text>
            <VideoPlayer
              videoProps={{
                shouldPlay: true,
                resizeMode: "contain",
                source: require('../assets/quidEst.mp4'),
              }}
              fullscreen={{
                visible: false,
              }}
              style={styles.video}
            />
          <Text style={{fontWeight:'bold', marginTop:25, fontSize:16}}>"Quid" means "what" and "est" means "is."</Text>
...
export default SelectAndListenScreen1;